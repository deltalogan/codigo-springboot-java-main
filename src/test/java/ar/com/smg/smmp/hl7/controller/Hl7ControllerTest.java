...
class Hl7ControllerTest {
    ...
    @SuppressWarnings("unused") @MockBean private Hl7Service service;
    private static final String JSON = "{}";
    @Test
    void elegibilidad_deberiaRetornar200OK() throws Exception {
        when(service.elegibilidad(any())).thenReturn(new HL7ElegibilidadResp());
        mockMvc
                .perform(post("/api/elegibilidad")
    ...
    void anulaDiagnostico_deberiaRetornar200OK() throws Exception {
        ...
        mockMvc
                .perform(post("/api/anula-diagnostico")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(JSON))
                .andExpect(status().isOk());
    }
    ...
    void informaGeoposicion_exito_deberiaRetornar200() throws Exception {
        when(service.informaGeoposicion(any())).thenReturn(new HL7Geoposicion());
        mockMvc
                .perform(post("/api/informa-geoposicion")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(JSON))
                .andExpect(status().isOk());
    }
    ...
    void reimprimirComprobante_deberiaRetornar200() throws Exception {
        ...
        mockMvc.perform(get("/api/comprobantes/100")).andExpect(status().isOk());
    }
    @Test
