...
class Hl7ServiceTest {
    ...
    void informaDiagnostico_deberiaParsearCabeceraYDetalleCorrectamente() {
        ...
        HL7InfoDiagnosticoResp resp = service.informaDiagnostico(informaDiagnostico);
    ...
    void consultaAuto_deberiaParsearMultipleResultMap() {
        List<Object> resultMaps = List.of(List.of(new HL7ConsultaAutoCab()), List.of(new HL7ConsultaAutoDetalle()));
    ...
    void consultaTotales_deberiaParsearCabeceraYDetalle() {
        List<Object> resultMaps = List.of(List.of(new HL7ConsultaTotalesCab()), List.of(new HL7ConsultaTotalesDetalle()));
    ...
    void cancelacionPrestaciones_deberiaParsearRespuesta() {
        List<Object> resultMaps = List.of(List.of(new HL7CancelPrestacCab()), List.of(new HL7CancelPrestacDetalle()));
    ...
    void registracionProd_deberiaParsearRespuesta() {
        List<Object> resultMaps = List.of(List.of(new HL7RegProdCab()), List.of(new HL7RegProdDetalle()));
    ...
    @Test
    void consultaGeoposicion_cuandoNoExiste_deberiaLanzarHl7InexistenteException() {
        when(mapper.consultaGeoposicion(999)).thenReturn(null);
        assertThrows(Hl7InexistenteException.class, () -> service.consultaGeoposicion(999));
    }

    @Test
    void informaGeoposicion_transaccionNoExiste_deberiaLanzarExcepcion() {
        when(mapper.consultaTransaccion(isNull())).thenReturn(null);   // ← Corregido
        assertThrows(Hl7InexistenteException.class, () -> service.informaGeoposicion(geoposicion));
    }

    @Test
    void informaGeoposicion_yaExisteGeoposicion_deberiaLanzarHL7DuplicateException() {
        when(mapper.consultaTransaccion(isNull())).thenReturn(new HL7ConsultaTransaccionResp());
        when(mapper.consultaGeoposicion(isNull())).thenReturn(new HL7Geoposicion());
        assertThrows(HL7DuplicateException.class, () -> service.informaGeoposicion(geoposicion));
    }

    @Test
    void informaGeoposicion_exitoso_deberiaGuardarYRetornar() {
        when(mapper.consultaTransaccion(isNull())).thenReturn(new HL7ConsultaTransaccionResp());
        when(mapper.consultaGeoposicion(isNull())).thenReturn(null).thenReturn(new HL7Geoposicion());
    ...
    void reimprimirComprobante_deberiaRetornarListaEnvueltaCorrectamente() {
        List<Object> mockResult = Arrays.asList(
                List.of(new Object()), // cabecera
                List.of(new Object())  // detalle
        );
    ...
    void consultaPrestacionExceptuada_conParamNull_deberiaLanzarExcepcion() {
        assertThrows(SMMPRuntimeException.class, () -> service.consultaPrestacionExceptuada(null));
    }

    @Test
    void consultaPrestacionExceptuada_deberiaRetornarLista() {
        List<HL7ConsultaPrestacionExceptuada> expected = List.of(new HL7ConsultaPrestacionExceptuada());
        when(mapper.consultaPrestacionesExceptuadas(any())).thenReturn(expected);
        List<HL7ConsultaPrestacionExceptuada> result = service.consultaPrestacionExceptuada(new HL7ConsultaPrestacionExceptuada());
